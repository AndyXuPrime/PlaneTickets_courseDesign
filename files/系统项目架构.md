...
*   **前端**：Vue 2.7 + Element UI + Axios + Vue Router + Vue.observable + ECharts
...
后端采用 **Maven 多模块架构**。父工程管理依赖版本，子模块分为“公共库”和“独立微服务”。当前根 `pom.xml` 已聚合 `common-module`、`auth-service`、`flight-service`、`order-service`、`gateway-service`；`admin-service` 继承父工程配置，可单独构建与启动。
...
│       ├── controller/                   (FlightController, AirlineController, FileController)
│       ├── entity/                       (Flight, Airline)
│       ├── repository/                   (FlightRepository)
│       ├── service/                      (FlightService, PricingStrategy)
│       ├── service/impl/                 (FlightServiceImpl, DynamicPricingStrategyImpl)
...
6.  **后台服务 (`admin-service`)**：
    *   **角色**：数据分析中心。
    *   **核心功能**：直接连接业务数据库执行统计聚合查询（如总营收、热门航线），并提供系统广播消息接口。
...
│       ├── AdminView.vue         (后台管理布局)
│       ├── admin/                (Dashboard、航班管理、用户管理、票务中心、审核、航司管理、消息发布)
